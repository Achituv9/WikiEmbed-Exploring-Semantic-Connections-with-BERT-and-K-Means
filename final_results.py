...
top_50_categories = pd.read_csv("category_louvain_network_cluster.csv")  # top 50 categories
...
for i in range(2, 26):
    ...
    for category in top_50_categories["Category"]:
        ...
        result_df[f"{category}_adjusted"] = (
            result_df[category] / result_df["Cluster_Size"] * avg_cluster_size
        )

        total_category_adjusted_count = result_df[f"{category}_adjusted"].sum()
        result_df[f"{category}_adjusted_percent"] = (
            result_df[f"{category}_adjusted"] / total_category_adjusted_count
        )
    ...
    for category in top_50_categories["Category"]:
        columns.extend(
            [
                category,
                f"{category}_percent",
                f"{category}_adjusted",
                f"{category}_adjusted_percent",
            ]
        )
