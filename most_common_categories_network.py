...
def extract_text_and_categories(article_text):
    ...
    # e.g. "Category:1920 births Category:2008 deaths..." 
    # -> ['', '1920 births ', '2008 deaths ...']
    # The first split item is often empty if it starts exactly at 'Category:'
...
if not SKIP_PHASE_1:
    ...
    for filename in os.listdir(ORIGINAL_FILES_DIR):
        ...
        for article in data:
            ...
            # 1) Extract the portion of text before "Category:" 
...
community_ids = partition.values()
...
for (u, v, w) in G.edges(data=True):
    # scale the width by weight if desired
    edge_widths.append(0.1 + 0.9 * (w["weight"] / max(co_occurrences.values())))
...
plt.title("Co-occurrence Network of Top 50 Categories (Louvain Clustering)", fontsize=16)
plt.axis('off')
plt.tight_layout()
...
print(f"CSV with top 50 categories, counts, and cluster saved as '{TOP_CATEGORIES_CSV}'.")
